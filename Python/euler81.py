...
for i in range(matrix_size-2,-1,-1):
    final_matrix[matrix_size-1,i]+=final_matrix[matrix_size-1,i+1]#Adding Number to the Right
    final_matrix[i,matrix_size-1]+=final_matrix[i+1,matrix_size-1]#Adding Number to the Bottom

for row in range(matrix_size-2,-1,-1):
    for col in range(matrix_size-2,-1,-1):
        #current_val=final_matrix[row,col]
        final_matrix[row,col]+=min(final_matrix[row+1,col],final_matrix[row,col+1])#current_val+min(right,bottom)
